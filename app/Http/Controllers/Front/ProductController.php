...
class ProductController extends Controller
{
    ...
    {
        $data=[];
        $data['product'] = ProductModel::where('slug',$slug) -> first();  //improve select only required fields
       ...
       $data['product_attributes'] =  AttributeModel::whereHas('options' , function ($q) use($product_id){
            $q -> whereHas('product',function ($qq) use($product_id){
                $qq -> where('product_id',$product_id);
            });
        })->get();

         $data['related_products'] = ProductModel::whereHas('categories',function ($cat) use($product_categories_ids){
